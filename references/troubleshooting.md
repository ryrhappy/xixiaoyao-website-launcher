...
| Local build fails | First error in complete build output | Fix locally before GitHub or Netlify |
| Works locally, fails on Netlify | Node version, base directory, env, build/output settings | Reproduce with the same production build |
...
| Supabase works locally only | Netlify env scope and redirect URLs | Add correct deploy-context values and redeploy |
| Domain says invalid configuration | Actual authoritative DNS and conflicting records | Read Netlify's current domain status; change records at the real DNS host |
